...
namespace Orchard.Roles.Services
{
    public class RoleService : IRoleService
    {
        ...
        public void UpdateRole(int id, string roleName, IEnumerable<string> rolePermissions)
        {
            ...
            var groupedPermissions = roleRecord.RolesPermissions.GroupBy(x => x.Permission.Name);
            var currentPermissions = groupedPermissions.Select(x => x.FirstOrDefault()).ToDictionary(x => x.Permission.Name);
            
            // Log error if there are duplicate role permission combination
            if(groupedPermissions.Any(x => x.Count() > 1))
            {
            ...
            foreach (var rolePermission in rolePermissions)
            {
                ...
                if (!roleRecord.RolesPermissions.Any(x => x.Permission == permissionRecord && x.Role == roleRecord))
                {
                    roleRecord.RolesPermissions.Add(new RolesPermissionsRecord { Permission = permissionRecord, Role = roleRecord });
                }
